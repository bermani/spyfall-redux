...
function generateNewGame(deck){
  var game = {
    accessCode: generateAccessCode(),
    state: "waitingForPlayers",
    deck: deck,
    decklist: locations[deck].sort( function( a, b ) {
    return a.name < b.name ? -1 : a.name > b.name ? 1 : 0;}),
    deckname: deck_names[deck],
    ss: false,
    spy_name: null,
    location: null,
    lengthInMinutes: 8,
...
function trackGameState () {
  ...
  if(game.state === "inProgress"){
    ...
    Session.set("beenInGame",true)
  } else if (game.state === "waitingForPlayers") {
    if (Session.get("beenInGame")) {

    if (game.ss) {
      swal("Secret Spy Round!","Everyone was the spy!")
    } else {
      swal({title:game.spy_name+" was the spy!",text:"The location was: "+game.location.name
  });
    }
...
Template.main.rendered = function() {
  $.getScript("//pagead2.googlesyndication.com/pagead/js/adsbygoogle.js", function() {
    var ads, adsbygoogle;
    ads = '<ins class="adsbygoogle" style="display:block;" data-ad-client="ca-pub-9814747448554174" data-ad-format="auto"></ins>';
    $('.adspace').html(ads);
...
Template.startMenu.events({
  ...
  'click #btn-how-to-play': function () {
...
Template.howToPlay.events({
  ...
  'click .btn-strategy': function () {
    Session.set("currentView", "strategyInfo");
    scroll(0,0);
  }
});

Template.strategyInfo.events({
  'click .btn-back': function () {
    Session.set("currentView", "howToPlay");
    scroll(0,0);
  },
...
Template.createGame.events({
  'submit #create-game': function (event) {
    ...
    var deck = event.target.deck.value;

    if (!playerName || Session.get('loading')) {
      return false;
    }

    var game = generateNewGame(deck);
