...
function getRandomLocation(deck){
  var chosen_deck = locations[deck];
  var locationIndex = Math.floor(Math.random() * chosen_deck.length);
  return chosen_deck[locationIndex];
}
...
Games.find({"state": 'settingUp'}).observeChanges({
  added: function (id, game) {
    var location = getRandomLocation(game.deck);
    var spy_school = Math.random()<0.05;
    var spyname = null;
    ...
    players.forEach(function(player, index){
      Players.update(player._id, {$set: {
        isSpy: index === spyIndex || spy_school,
    ...
    Games.update(id, {$set: {state: 'inProgress', location: location, ss: spy_school, spy_name: spyname, endTime: gameEndTime, paused: false, pausedTime: null}});
  }
});
